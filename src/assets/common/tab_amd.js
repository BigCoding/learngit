 ...
 * }
 */
define(['text!assets/components/tab/tabLoad.tpl','jquery.placeholder'],function(load){
	//创建tab对象
	function Tab(options){
		...
		this.settings=$.extend({
			...
			data:[],
			colNum:15,//下拉列表每列显示个数
			colWidth:90,//下拉列表li宽度
			closeable:true,
			isDisabled : false,
			isLazy:true,
	...
	Tab.prototype = {
		...
		createTab : function(options, businessOptions){
			return new TabItem(this,options, businessOptions);
		},
		/***
	...
	TabItem.prototype = {
	  /**
		...
		initDom:function(){
			...
			this.$label=$("<span class='tilteText'></span>").appendTo(this.$innerWrap);
			...
			function afterRemove(){
				...
				if(self==o.curItem){ //如果关闭的tab是激活的tab，则根据策略激活新tab
					...
					switch(o.settings.tabActiveMode){
						case "next":willShow=(o.items[closeTabIndex])?o.items[closeTabIndex]:o.items[closeTabIndex-1];break;
		...
		load:function(url){
			...
			if(this.data.isFrame){
				...
				try{
					...
					// .css({"width":"100%","height":$(window).height()-152});
				} catch(e){}
				var timer=null;   //iframe自适应高度的timer
				//isAdjustHeightByContent 在本系统中无用，故禁用
				// function timerFn(){
				// 	var i=$(item.iframe);
				// 	if(i){
				// 		try{
			...
			// requirejs模式
				...
				require([_url], $.proxy(function(objClass){
					//这段逻辑支撑了所返回的各种模块定义
					if (typeof(objClass) === 'function'){
						this.events = {};
						var obj=(this.data && this.data._index || {});
						var result = new objClass(obj, this.businessOptions,this);
						if (typeof(result) === 'object'){
							if (result.hasOwnProperty('content')){
								var $box = $('input,textarea',result.content);
								$box.placeholder && $box.placeholder();
								self.empty().append(result.content);
							}else{
								self.empty().append(result);
							}
							this.trigger("renderCallback");
						}else{
							self.html(result);
						}
					}else{
