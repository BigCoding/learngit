define(['Util',
    'text!assets/components/voice/voice.tpl',
    'style!assets/components/voice/voice.css',
    'style!assets/lib/jqueryPlugin/jPlayer/dist/skin/flat.tony/css/jplayer.flat.tony.css',
    'jquery.jplayer'
], function(Util, tpl) {
    ...
    var objClass = function() {
        ...
        if (options.el instanceof jQuery && options.el.length > 0) {
            ...
            window["console"] && console.log('配置项el不存在');
            this.$el = $("<div></div>")
        }
        // 判断是否有className
        ...
        // 判断是否有repeat
        options.repeat = (options.repeat ? options.repeat : false);
        // 判断是否有shortcutKey
        options.shortcutKey = (options.shortcutKey ? options.shortcutKey : true);
        ...
        setTimeout($.proxy(function() {
            this.$jplayer.jPlayer({
                ...
                swfPath: options.swfPath ? options.swfPath : "src/assets/lib/jqueryPlugin/jPlayer/dist/jplayer",
                solution: "flash, html",
                supplied: "mp3",
